...
export default function App() {
  ...
  const handleLogin = async (cred) => {
    ...
    try {
      ...
      const allowedEmails = data.config?.allowedEmails || [];
      // Fallback: also check env variable for first-time setup
      const fallbackEmail = import.meta.env.VITE_ALLOWED_EMAIL;
      const isAllowed = allowedEmails.includes(decoded.email.toLowerCase()) || decoded.email === fallbackEmail;
      if (!isAllowed) {
          ...
          <div style={{ display: "flex", minHeight: "100vh" }}>
            ...
            <div style={{ flex: 1, overflow: "auto", background: "#0f0500" }}>
              ...
              <div style={{
                ...
                <div>
                  <h2 style={{ fontFamily: "'Cinzel', serif", fontSize: "1rem", color: "#faf5e8", fontWeight: 600 }}>
                    {section === "notice" ? "📢 Notice Ribbon" : section === "events" ? "📅 Events" : section === "locations" ? "📍 Locations" : "🔐 Access"}
                  </h2>
                  <p style={{ color: "#5a3a1a", fontSize: "0.72rem", marginTop: "2px" }}>
                    {section === "notice" ? "Control the scrolling notice bar" : section === "events" ? "Manage events on the website" : section === "locations" ? "Manage darbar locations" : "Control who can access this admin panel"}
                  </p>
                </div>
                <div style={{
              ...
              <div style={{ padding: "32px" }}>
                ...
                {section === "access" && <AccessAdmin currentUserEmail={user?.email} />}
              </div>
            </div>
          </div>
        )}
